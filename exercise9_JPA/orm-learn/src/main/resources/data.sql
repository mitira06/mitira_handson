-- Auto-executed by Spring Boot on startup (since we're using H2 + ddl-auto=create-drop,
-- Hibernate creates the table from the @Entity, then this file seeds sample data).
-- Full world country list (from Hands-on 5 of Doc 1), needed for Query Methods testing in Doc 2.
...
-- Doc 2, Hands-on 3-6: payroll schema seed data (Department, Skill, Employee, employee_skill)
-- payroll.sql from the course platform was not available, so this data is generated to match
-- the exact column names/structure described in the hands-on docs (em_*, dp_*, sk_*, es_*).
...
-- em_id=1 -> department 1 (Engineering) - the only employee in dept 1, used in Doc 2 Hands-on 4 testGetEmployee()
...
-- em_id=2,3 -> department 2 (Human Resources) - matches Doc 3's own example query (dp_id=2, em_id=2)
...
-- em_id=4,5 -> department 3 (Finance) - matches Doc 3's own example query (dp_id=3)
...
-- em_id=6 -> department 3, not permanent (used to demonstrate findByPermanent filtering)
...
-- employee_skill (many-to-many join table: es_em_id, es_sk_id)
-- Employee 2 (Priya) has Java + SQL -- referenced directly in Doc 3's own example query (es_em_id=2)
...
-- Employee 3 and Employee 6 intentionally have NO skills yet, and Employee 3 has no
-- relationship with Skill 6 ("Project Management") -- used in Doc 2 Hands-on 6's
-- testAddSkillToEmployee(), which looks for an employee/skill pair with no existing relationship.
