...
|---|---|---|
| MySQL Server 8.0 | **H2 in-memory** | You don't have MySQL installed. H2 needs zero setup — it runs inside the app. All the JPA/Hibernate code is identical either way. |
| Manually run `mysql -u root -p` + `create schema` | Not needed | H2 in-memory database is created automatically when the app starts. |
| `spring.jpa.hibernate.ddl-auto=validate` | `create-drop` | `validate` requires the table to already exist in the DB. Since H2 is fresh every run, `create-drop` lets Hibernate create the table from `Country.java` automatically, then drop it on shutdown. |
| Manually run `insert into country values (...)` | `src/main/resources/data.sql` | Spring Boot auto-runs this file against H2 on every startup — same two rows (`IN`/India, `US`/United States) as the doc. |
| `co_code` / `co_name` column names in the raw SQL section | `code` / `name` | The doc's own `Country.java` example maps `@Column(name="code")` / `@Column(name="name")`, not `co_code`/`co_name` — that mismatch in the original doc. I matched the Java class, since that's what Hibernate actually uses. |

## Switching to MySQL later

1. Make sure MySQL is running and you've created the `ormlearn` schema (see the doc's instructions).
2. In `application.properties`, comment out the H2 block and uncomment the MySQL block at the bottom.
3. Change `mysql.password`/`username` to match your local MySQL.
4. Since `ddl-auto=validate` is used for MySQL, you must create the `country` table yourself first
   (the doc gives the `create table country(...)` statement) — Hibernate will only check it matches, not create it.
